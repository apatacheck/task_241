...
#### 6) Чем отличаются stdout и stderr
  ...
  **STDERR (standard error)** — это стандартный поток ошибок, который используется программами для вывода диагностических и отладочных сообщений. По умолчанию сообщения из `stderr` выводятся на экран терминала. Соответствует дескриптору 2.
    
Буфер представляет собой область оперативной памяти, в которую программа временно записывает данные перед их выводом. Это позволяет уменьшить количество системных вызовов и ускорить работу программы. При выводе в терминал **STDOUT**, как правило, использует построчную буферизацию, при которой данные выводятся после символа перевода строки, а при выводе в файл или при использовании конвейеров применяется блочная буферизация, когда данные передаются на устройство вывода после заполнения буфера.
  В отличии от **STDOUT**, **STDERR** чаще всего не буфезируется или использует минимальную буферизацию, это значит что все ошибки выводятся немедленно.
